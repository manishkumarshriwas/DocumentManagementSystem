...
import { registerUser, validateRegistrationOtp } from '../services/api';
import { validateMobileNumber, validateEmail, validatePassword } from '../utils/validation';
...
const Register = () => {
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === true || response.success === true) {
        ...
        setRegistrationStep(2); // Move to OTP verification step
        setShowOtpSection(true);
        setTimer(120); // Reset timer
      } else {
        const errorMessage = response.data || response.message || 'Registration failed';
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      const response = await validateRegistrationOtp(formData.mobileNumber, otpValue);
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        ...
        <p className="mt-2 text-center text-sm text-gray-600">
          {registrationStep === 1 
            ...
            : "We've sent a verification code to your mobile phone"}
        </p>
      </div>
